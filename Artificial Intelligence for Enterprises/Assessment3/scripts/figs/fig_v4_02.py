...
🔴 2026-08-09 依 `notes/15_Kenny第二批答覆_全案重算.md`(位階最高)全面重畫:
   ...
   · 帶 2 五個問號**全部填上 Q45 的時間拆解**,並標出 AI 可處理段佔全程 43%-78%
     (🔴 2026-08-12 依裁決 P-① 更正:舊值 60%-64% 是上下界同步配對得到的假精確,已作廢)
...
🔴 2026-08-09 第二輪(方案升級為地端 VLM 事件判讀,依 `notes/16_地端VLM方案升級_裁決與待辦.md`
   ...
   4. 🔴 **帶 2 五段由等寬改為按天數比例**(規格【排版要求】早就這樣寫,舊版沒做到):
      等寬時 AI 可處理段在畫面上只佔 2/5 = 40%,而它正下方的標註寫 43%-78% ——
...
- 🚫 作廢數字一個都不得出現:4.4 分 · 120 人時 · 27.7-38.7 · 69%-97% · 3.5-4.8 人 ·
  ...
  60%-64% · 36%-40% · 9%-14%(後三者於 2026-08-12 依裁決 P-① / Z-8 作廢)。
"""
import os
import sys
...
# 段 2 + 段 3 = 2.5 / 4.0 = 62.5%,落在標註的 43%-78% 內 —— 長度即論證,不得為了塞字改比例。
BX0, BX1 = 0.030, 0.556
...
def _breakdown(ax):
    ...
    ax.text(L1, 0.6535, "非 AI 段(排隊 + 覆核 + 回信)只佔 22%-57%,",
            ha="right", va="top", fontsize=10, color=INK2, zorder=6)
