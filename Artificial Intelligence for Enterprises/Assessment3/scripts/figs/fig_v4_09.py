...
🔴 本輪主改(視覺規格【AI 主包視覺加重】①~⑦):
  ...
  ⑤ 加註兩段(AI 主體宣告 + 主線模型 + 影子模式聲明)—— 見下方 ⚠️ 位置說明。
     🔴 2026-08-12 裁決 X:第三行舊寫「AI 不做判定…判定仍由人做」已作廢(全片禁止),
        改為「機器會做結案判定 —— 但本案六個月內不對外生效(影子模式)」;
        同輪 G2 由「三方比較門 / 三組」改為「四臂比較門 / 四臂」(裁決 W),M5 交付名同步。
...
AI_NOTE = [
    ...
    ("模型與量化規格於 G1 依 W1-6 量測定案(概念層次)", 11, NAVY, "normal"),
    # 🔴 2026-08-12 裁決 X:舊句「AI 不做判定…判定仍由人做」在讀法(i) 下是假的,
    #    且是 Codex 判 REFUTED 的紅線,全片禁止再出現。取代語見下。
    ("機器會做結案判定 —— 但本案六個月內不對外生效(影子模式)。", 9, GREY, "normal"),
]
...
MOVED_TO_SLIDE = [
    ...
    ("P9-T2", "圖下方左欄 · 決策門說明卡(三段並排或三段條列)", [
        ...
        "G2 · W12 四臂比較門:在同一凍結評估集上同時比四臂:①現行影像模型 ②規則式脈絡基線 "
        "③人機協作(機器輔助、人判定)④影子模式 —— 機器自己結案但不對外送,人照常逐筆判,兩邊比對。"
        "第四臂是唯一算得出「自動結案漏放率」的設計:前三臂全部含人,而要部署的是無人系統。"
        "成功條件:人機判定品質不劣於現況 —— 以自動結案桶與人工桶的漏放率差值建 95% 信賴區間,"
        "上界不得超過 G1 事前預註冊的非劣性邊際 δ;且人工觸碰時間與 P90 回覆時間達預註冊門檻。"
        "本門只能調整模型的假設與規模,不能取消實驗。"
        "凍結評估集、預註冊門檻與不可取消實驗三項一字不動(裁決 K)。"
        "規則式若已解掉大部分誤報,靶心改為殘餘(情境模糊與判準爭議);"
        "若無效,先查脈絡資料本身的品質,再決定模型端的規模。"
        "「自動結案要不要對外開啟」不由 G2、也不由 G3 決定,它需要移交後一道獨立的門,"
        "不在本次請求範圍。",
    ...
    ("P9-T3", "圖左欄延伸 · 六個工作包的副標與交付(緊貼圖左側或置於圖下方,順序同圖)", [
        ...
        "｜交付:四臂對照結果(現行影像模型 / 規則式脈絡基線 / 人機協作 / 影子模式)"
        "+ 有脈絡 vs 無脈絡的對照結果;W12-24 只跑影子模式 —— 機器自己結案但不對外送",
