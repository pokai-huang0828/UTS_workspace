...
留在圖裡的(靠位置/連線/顏色/比例傳達意義,拆掉就講不清楚):
  左:人月矩陣(6 工作包 × 5 角色 + 小計 / 角色合計 / PM / 總計,色階 + 斜線紋)
      + 斜線紋與「內部前移」一行圖例
      ⚠️ WP-E 列名改為「地端 VLM · 本案的 AI 主體」並加橘框加重(與計畫頁 AI 主包同一包)
  右:RACI 六列 × 九欄(A 實心深藍圓、R 橘框圓、C/I 淺灰)+ 字母圖例
      + 相依帶(雙軌 + 待共同確認箭頭 + 實虛線圖例)+ 橘色母題(不加文字)
  底:🔴 本輪新增 —— 已估成本構成條(滿版堆疊條:橘 = 人力 88%-92% / 深藍 = 設備 8%-12%)。
      8%-12% 是**比例**,靠長度傳達比印一行字有效,依裁決 O 的留圖判準留在圖裡;
      框三其餘純文字(14%-19% 對照、採購時點、佔用率、市場採購量級性質、技術問答兩句)移出。

y >= RESERVED_TOP(4.35 吋)的底帶刻意留白,留給組版程式疊原生文字框
(三個算式框 + 放行條件框移出圖的落點)。_check_layout() 會 assert 這條帶上沒有任何文字。

移出圖、改由組版程式放的(全部是純文字,靠左對齊的 × / = 不傳達圖形關係;
完整逐字清單見交付回報的 moved_to_slide,一個字都不能少):
  算式框一、算式框二、**算式框三(地端設備)除堆疊條已承載的三項數字外的全部字**、
  **第二個應用(技術問答)兩句話**、放行條件框、三框下方三行註記與框間分隔說明、
  箭頭上的反事實說明、利害關係人窄帶、六個月後三格帶 + Q-22、Q-19、組織障礙一行、
  相依帶下方的請求行、WP-E 待確認註記全文、第一段人月拆解算式全文、
  RACI 六方當責的完整職稱與複核營運主管註記、結論條三行。
...
WPS = [
    ...
    # WP-E = 本案的 AI 主體(地端 VLM 事件判讀,原對照重訓那一包);
    # 列名欄只有 1.40 吋可用,完整名稱「地端 VLM 事件判讀(對照重訓)」放不進去,
    # 折成兩行 + 加重外框標示,完整名稱由組版程式的投影片文字承接。
    ("WP-E 地端 VLM W8-24\n本案的 AI 主體 · 對照重訓",
     {0: ("內部前移", "3.00"), 1: ("內部前移", "1.50")}, "4.50", False),
...
def build():
    ...
    # 左欄上 —— 人月矩陣
    # ==============================================================
    # 列名欄由 1.42 加寬到 1.52(WP-E 要放得下兩行「地端 VLM / 本案的 AI 主體」),
    # 差額自角色欄取(0.70 -> 0.68,格內最寬的「內部前移」仍只有 0.50 吋)。
    cw_name, cw_role, cw_sub = 1.52, 0.68, 0.93     # 1.52 + 3.40 + 0.93 = 5.85
    ...
    # 各列高較上一版各縮 0.01-0.02 吋:騰出底部 0.4 吋給本輪新增的成本構成條。
    # 🔴 縮的是列高不是字級 —— fs 一律維持 >= 9(裁決 O 第 2 條)。
    yc = 0.28
    h_head, h_wp, h_sum, h_tot = 0.28, 0.37, 0.28, 0.38
    ...
    for name, cells, sub, seg1 in WPS:
        is_ai = "AI 主體" in name          # WP-E:全案唯一的 AI 主包,列名加重
        if is_ai:
            # 加重只走外觀(橘框 + 橘字),週次與人月一個字都不動,與計畫頁同一手法。
            rect(ax, L0, yc, cw_name, h_wp, ORANGE_LIGHT, ec=ORANGE_DARK, lw=1.6)
            line(ax, L0 + 0.06, yc + h_wp / 2, name, fs=FS_TINY, c=ORANGE_DARK,
                 bold=True, va="center", lsp=1.25)
        else:
        ...
        for i in range(5):
            if i in cells:
                ...
                line(ax, xs[i] + cw_role / 2, yc + h_wp / 2 - 0.090, top, fs=FS_TINY,
                     c=top_c, ha="center", va="center")
                line(ax, xs[i] + cw_role / 2, yc + h_wp / 2 + 0.090, val, fs=FS_BODY,
    ...
    # 圖例(解釋圖面上的斜線紋與「內部前移」兩個記號,屬圖不屬文,留在圖裡)。
    # 上一版是兩行,本輪壓成一行以騰出成本構成條的高度;被壓掉的細節
    #(第一段範圍 = WP-A 全部 + WP-B / WP-C 限額探索)本來就已在投影片文字裡。
    line(ax, L0, yc + 0.04,
         "斜線紋 = 第一段(W1-6)的工作包　·　"
         "「內部前移」= WP-E 內部重排人力,總投入 4.50 人月不變",
         fs=FS_TINY, c=GREY)
    ...
    # 🔴 h_rr 由 0.325 縮為 0.29:視覺規格明訂「若仍擠不下,優先縮的是 RACI 表的列高」,
    # 六列共讓出 0.21 吋給底部的成本構成條。表頭 h_rh 不能再縮(三行欄名 fs=9 就要 0.48 吋)。
    ry, h_rh, h_rr = 0.34, 0.50, 0.29
    ...
    # 右欄下 —— 相依帶(雙軌 + 待共同確認箭頭)
    # ==============================================================
    up_y, lo_y, rail_h = 2.93, 3.44, 0.27
    ...
    # 圖例改右對齊(視覺規格:「圖例置於相依帶右下角」),同時讓開左欄那條加長的斜線紋圖例。
    line(ax, R1, lo_y + rail_h + 0.08,
         "實線 = 本提案要交付的　　虛線 = 尚未定案", fs=FS_TINY, c=GREY, ha="right")

    # 全片最後一次出現的橘色母題(人工複核防線,不加文字)。
    # 上一版貼在相依帶右下角,本輪那個位置被成本構成條的右側註記佔走,改貼兩軌之間的右緣空白。
    roundbox(ax, R1 - 0.22, up_y + rail_h, 0.22, 0.20, ORANGE, ORANGE_DARK,
             lw=1.2, z=6, register=False)

    # ==============================================================
    # 🔴 底部滿版 —— 已估成本構成條(本輪新增:地端設備入成本)
    # 為什麼這一條留在圖裡、而框三其餘的字移出去:8%-12% 是**比例**,靠長度一眼就讀完;
    # 印成一行字則要委員自己在腦裡做除法。裁決 O 的留圖判準正是「靠比例傳達意義」。
    # 🚫 這條**只准**呈現「人力 + 設備 = 已估成本」的構成,不得與算式框一(年度擴編暴露)
    #    相減 —— 那個減法在算術上不成立(機器只打得到 AI 可處理段那 43%-78% 裡的一部分,
    #    人仍須覆核)。🔴 2026-08-12 裁決 P-①:舊寫法「那六成」源自已作廢的 60%-64%。
    # ==============================================================
    bar_y, bar_h = 4.13, 0.21
    bar_w = R1 - L0                       # 12.00 吋:跨左右兩欄,是本頁唯一的滿版元素
    dev_frac = 0.10                       # 8%-12% 的中點,長度畫 10%,字面照實寫區間
    w_hr = bar_w * (1.0 - dev_frac)

    rect(ax, L0, bar_y, w_hr, bar_h, ORANGE)
    line(ax, L0 + 0.09, bar_y + bar_h / 2,
         "人力成本估算 NT$158-177 萬(88%-92%)　——　這個專案的成本主體是人,不是設備",
         fs=FS_TINY, c=DARK, bold=True, va="center")
    rect(ax, L0 + w_hr, bar_y, bar_w - w_hr, bar_h, NAVY)
    line(ax, L0 + w_hr + (bar_w - w_hr) / 2, bar_y + bar_h / 2, "設備 8%-12%",
         fs=FS_TINY, c="white", bold=True, ha="center", va="center")

    line(ax, L0, bar_y - 0.18,
         "已估成本構成〔提案推算〕　深藍 = 地端 128G 設備 NT$16-20 萬 / 台"
         "(一次性 · 先以一台起步)", fs=FS_TINY, c=NAVY, bold=True)

    # ==============================================================
    # 底帶 y >= 4.35 吋 —— 刻意留白,不畫任何東西
    # 三個算式框與放行條件框是純文字(靠左對齊的 × / = 不傳達圖形關係),依裁決 O 第 4 條
    # 移出圖,由 build_deck_v4.py 用 PowerPoint 原生文字框疊在這條空白帶上:
    #   置圖後這條帶對應投影片 y 約 5.35-6.75 吋、x 0.55-12.54 吋,
    #   多框並排、字級 11-12pt(原生向量),比烘進 PNG 的 9pt 更清楚。
    #   完整文字見交付回報的 moved_to_slide,一個字都不能少。
    #   ⚠️ 本輪移出的字量增加(框三 + 技術問答兩句),組版時底帶可能不夠 ——
    #      不夠時往投影片下緣延伸,🚫 不得縮小本圖或把字塞回圖裡。
    # ==============================================================
