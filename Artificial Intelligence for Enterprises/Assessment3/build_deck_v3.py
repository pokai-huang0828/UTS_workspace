...
def build():
    ...
    picture(s, "fig_v3_05_plan.png", top=TOP + 0.15, maxh=4.30)
    txt(s, 0.55, TOP + 4.58, W - 1.1 - SAFE_R, 1.2,
        "這八週裡,不會訓練任何模型。名單用 SQL 依到期日取,介入用現有外撥流程,唯一新增的是隨機分派與結果追蹤。\n"
        "五個工作包的交付物全部可簽收:CEE 覆蓋現況報告、到期分佈實算、流失事件操作型定義(業務與資料雙簽)、\n"
        "合規意見書、實驗結果報告、回委員會的建議書。",
        size=11.5, color=DARK, spacing=1.4)
    ...
    txt(s, 0.55, TOP + 3.80, W - 1.1 - SAFE_R, 1.9,
        "關鍵路徑是「合規預審 → 對照實驗」—— 合規意見書未出,實驗不能開始。\n"
        "這是本案唯一的外部依賴,也是唯一需要簽核才能放行的關卡。\n\n"
        "Planning estimate — to be calibrated with CBA internal rates。成本 = 3.5 人月 × 澳洲 ICT 職類薪資\n"
        "中位數區間 × 1.3 間接費率(間接費率為提案人假設)。無新增系統採購、無平台授權、無外部顧問。",
        size=11.5, color=NAVY, spacing=1.4)
