...
⚠️ 85% 準確率的三個要件(推估非實測 / 第 12 週凍結測試集驗 / 人現在判得多準沒量過)
   與第四段「判的是描述不是判定」整段移出圖(SLIDE_TEXT 的 P5-T15,fs>=10)。
...
CARDS = [
    dict(title="選項 0 · 加人", kind=0, motif=True,
         ...
         # 明寫斷行點,免得「16-23 人」被拆到兩行
         kf=("以現行做法覆蓋全部既有客戶:\n16-23 人(現有 5 人)", LIGHT, BODY)),
    ...
    dict(title="選項 2 · 地端 VLM 事件判讀", kind=2, motif=False,
         ...
         # 兩模型分工:圖上只放「哪個模型對哪條工作流」這個對應關係,
         # 量化級別(4-bit / 8-bit / 10-16 分)是純文字,移到 SLIDE_TEXT。
         extra=[("Qwen3.5 122B-A10B → 全量初篩", NAVY),
                ("Qwen2.5-VL 72B → 爭議深入判讀", NAVY)],
         kf=("三組共用同一份凍結評估集\nAI 整理證據,判定仍由人做", NAVY, "white")),
...
SLIDE_TEXT = [
    ...
    dict(id="P5-T04", slot="COL1", label="選項 0b · 三行要點(規格明訂:任何情況下不得縮字或省略)",
         text=["做什麼:重排班表把人力挪到尖峰時段 + 建一個可調用的尖峰人力池"
               ...
               "AI 可處理的兩段(找證據 0.5 天 + 判讀 1-3 天)佔全程 2.5-5.5 天的六成上下,"
               "而排隊本身只有 0-1 天。排班動得到的是排隊那一段,動不到那六成。"
    ...
    dict(id="P5-T06", slot="COL3",
         ...
         text=["做什麼:地端視覺語言模型(VLM)事件判讀:讓機器讀懂一次事件的完整脈絡"
               ...
               "同時比三組:(1) 現行影像模型 (2) 規則式脈絡基線 (3) 地端 VLM。"
    ...
    dict(id="P5-T15", slot="COL3",
         ...
         text=["① 以現行小模型的表現外推,預期描述準確率約 85%〔提案推估,非實測〕。",
               "② 這個數字要不要成立,由第十二週用同一份凍結測試集驗。",
               "③ 而人現在判得多準,我們沒有量過;所以我也不知道 85% 是進步還是退步,"
               "那正是第一個工作包要建的東西。",
               "④ 這 85% 判的是「影片內容的描述正不正確」,不是「判這筆是不是誤報」。"
               "AI 不做判定,AI 做的是把證據整理好;判定還是人做的,只是他不用再花半天"
               "去把證據找出來。15% 會描述錯,所以設計上人仍須覆核:五位分析人員不是被裁掉,"
               "是從逐筆看片變成處理難題與治理(這一條在倫理頁是同一句話)。",
               "〔紅字警語〕85% 是描述準確率,現有 10.1% 是偵測誤報率,兩者量的不是同一件事,"
    ...
    dict(id="P5-T10", slot="BAND", label="三條策略窄帶 · 第 1 格「(1) 脈絡特徵工程」",
         text=["靶心是某小型車隊單月樣本中 103 / 124 = 83% 的同一根因;對應以現行做法覆蓋全部既有客戶時,"
               "每年可避免的增聘成本 NT$1,144-2,106 萬〔提案推算〕。"
               "此為不做本案時的年度增聘成本暴露,非本案承諾的節省額。",
               "算式:增聘 11-18 人 x 年薪量級 x 間接費率。以公開薪資量級推算,非核定預算。",
