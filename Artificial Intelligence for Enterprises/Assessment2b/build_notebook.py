...
| notebook 章節 | 報告章節 | 項 |
|---|---|---|
| §1 特徵工程 | 第一節 | 不應考慮的變數識別 |
| §2 最優聚類數 | 第二節 | 最佳聚類數的識別 |
| §3 聚類的語言解釋 | 第三節 | 對聚類的語言學解釋 |
| §4 績效最優聚類 | 第四節 | 最佳表現聚類的識別 |
| §5 穩健性與限制 | 限制段 | (支撐前四節論證) |

## 可重現性

全檔固定 `RANDOM_STATE = 42`、`n_init = 10`。範本未設 `random_state`,
...
| k 的決定 | 直接示範 k=4 | 四項判準裁決後採 k=4 | 題目要求說明 k 的選擇理由 |
