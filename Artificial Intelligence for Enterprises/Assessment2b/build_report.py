...
BODY['管理摘要'] = [
    '本報告以 K-means 將 107 位 IBM 顧問依三項績效指標分為四群,作為獎金級距依據(圖五);分析排除員工識別碼並施以標準化。「帶隊核心群」(4 人)為績效最優群,惟僅佔 3.7%;「高投入待認可群」(25 人)投入度達平均 2.2 倍卻幾乎未獲指派,在未標準化的分析中無法辨識。',
...
BODY['一、主要數據特徵:納入與排除的決策'] = [
    '排除 EmployeeID:它是標稱型識別碼,數值不承載順序意義;且標準差為其餘三欄的 36 至 103 倍,納入後獨佔距離平方 99% 以上(表一)。保留該欄實跑的反證顯示,分群會退化為按編號切段。',
...
BODY['二、最優聚類數的決定'] = [
    ...
    '判準門檻亦經檢驗:取 3 或 4 人結論同為 k = 4(notebook §2.5c),惟該門檻無外部依據。k = 4 的種子平均調整後蘭德指數 0.961,僅 10 人曾變動且全在邊界,個體輪廓值無一為負(圖四)。',
...
BODY['三、四個群組的語言解釋'] = [
    ...
    '分受眾而言:人力資源部門可將四群對應四級獎金級距(圖七),公平性基礎在於級距由 KPI 實際分布劃分而非主觀判定;部門主管應注意專業骨幹是負責人職位的候補來源;高階主管須面對 96% 的顧問從未帶隊。',
...
BODY['四、績效最優群組的識別'] = [
    ...
    '第三節指出該二指標帶有「機會」成分,故須確認判定不倚賴它們:僅以使用率排序,群 1 仍為最高 0.810(notebook §4.2b),三種權重方案亦皆最高,成員在 20 組種子下未變動。惟其僅佔 3.7%,配置建議見圖七;群 2 若政策不將帶隊列為必要條件,可與群 1 併為最高級距。',

]

BODY['五、方法限制'] = [
    '序位變數被視為等距是本方法的固有限制:以秩轉換重跑,調整後蘭德指數為 0.38,群組邊界會隨處理方式移動,惟核心發現不變。另,資料為單一時點。',

]

BODY['結論'] = [
    '採用方案為:排除員工識別碼、施以標準化、以 k = 4 分群。標準化由認可度獨佔 79.6% 距離權重、分群退化為單變數複製、10 位顧問遭錯置三項證據支持;k = 4 由事前判準裁決並經敏感度檢驗。',
