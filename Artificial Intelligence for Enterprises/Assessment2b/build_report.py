...
BODY['二、最優聚類數的決定'] = [
    ...
    '判準門檻亦經檢驗:取 3 或 4 人結論同為 k = 4,惟該門檻無外部依據(notebook §2.5c)。k = 4 的種子平均 ARI 為 0.961,僅 10 人曾變動且全在邊界;個體輪廓值亦僅 1 人為負(圖四)。',

]

BODY['三、四個群組的語言解釋'] = [
    '分群於標準化空間進行,呈現時還原為原始單位(表四、圖二、圖六)。聚類為描述性技術,呈現群組間差異,不主張因果。',
    ...
    '分受眾而言,三者要處理的問題不同。人力資源部門面對級距與公平性:四群可對應四級獎金級距(圖七),辯護基礎在於級距由 KPI 實際分布劃分而非主觀判定。部門主管面對人力配置:專業骨幹是負責人職位的候補來源,而高投入待認可群幾乎未獲指派。高階主管面對結構性風險:96% 從未帶隊、領導人才集中於 4 人。',
...
BODY['四、績效最優群組的識別'] = [
    ...
    '第三節指出該二指標帶有「機會」成分,故須確認判定不倚賴它們:僅以使用率排序,群 1 仍為最高 0.810(notebook §4.2b),三種權重方案亦皆最高,成員在 20 組種子下未變動。群 2 若政策不將帶隊列為必要條件,可與群 1 併為最高級距(圖七)。',
