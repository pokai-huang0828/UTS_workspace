...
BODY['二、最優聚類數的決定'] = [
    ...
    '判準門檻亦經檢驗:取 3 或 4 人結論同為 k = 4,惟該門檻無外部依據(notebook §2.5c)。k = 4 的種子平均 ARI 為 0.961,僅 10 人曾變動且全在邊界,個體輪廓值無一為負(圖四)。',
...
BODY['五、方法限制'] = [
    '序位變數被視為等距是本方法的固有限制:以秩轉換重跑,ARI 僅 0.38,群組邊界會隨處理方式移動,惟核心發現不變。資料亦僅為單一時點。',
