...
BODY['管理摘要'] = [
    '本報告以 K-means 將 107 位 IBM 顧問依三項績效指標分為四群,作為獎金級距依據(圖五)。「帶隊核心群」(4 人)為績效最優群,惟僅佔 3.7%;「高投入待認可群」(25 人)投入度達平均 2.2 倍卻幾乎未獲指派,在未標準化的分析中無法辨識。',
...
BODY['一、主要數據特徵:納入與排除的決策'] = [
    '排除 EmployeeID:它是標稱型識別碼,數值不承載順序意義;且標準差為其餘三欄的 36 至 103 倍,納入後獨佔距離平方 99% 以上(表一)。保留該欄實跑的反證顯示分群會退化為按編號切段。',

    '第二層決策是尺度。常見誤解是「不標準化較中立」,事實相反:未縮放時權重由各欄離散程度決定而非由業務判斷決定——認可度一欄即佔距離平方的 79.6%,使分群退化為單變數複製,與「直接按認可度分組」的調整後蘭德指數(Hubert & Arabie, 1985)達 0.99。',

    '其影響可具體量化(圖三、表二):10 位使用率 0.61 至 0.92 但認可度為 0 的顧問,未縮放時被併入一個使用率跨幅 0.00 至 0.92 的 86 人群。z 分數與 MinMax 產生相同分群(Milligan & Cooper, 1988)。標準化並非「不加權」,而是把權重改為可依政策調整的假設。',
...
BODY['二、最優聚類數的決定'] = [
    ...
    '統計面兩項衝突:輪廓係數最高在 k = 5(0.6823),但 k = 3、4、5 全距僅 0.0146、視為平手;肘部法指向 k = 4(圖一)。業務面則排除相鄰候選:k = 5 產生兩個各 2 人的群;k = 3 的中間群 27 人,群內認可度全距等於全體全距的 100%,獲指派 4 個專案者與未獲指派者同級,該指標在此級距內無區分力,k = 4 拆開後降至 50% 以下。',

    '判準門檻亦經檢驗:取 3 或 4 人結論同為 k = 4,惟該門檻無外部依據(notebook §2.5c)。k = 4 的種子平均 ARI 為 0.961,僅 10 人曾變動且全在邊界;個體輪廓值亦僅 1 人為負(圖四),分群品質良好。',

]

BODY['三、四個群組的語言解釋'] = [
    '分群於標準化空間進行,呈現時還原為原始單位(表四、圖二、圖六)。聚類為描述性技術,以下呈現群組間差異,不主張因果。',

    '四群的整體特徵如下。低度參與群共 70 人、佔全體近三分之二,三項指標均為最低,使用率僅 0.102,是組織的多數常態。高投入待認可群共 25 人,使用率 0.674 達平均 2.2 倍,但認可度僅 0.36 且無人帶隊,是三項指標落差最大的一群。受認可的專業骨幹共 8 人,使用率與認可度均居次高,惟尚未擔任負責人。帶隊核心群共 4 人,三項指標同時最高,是唯一實際帶隊的群組。四群的區隔並非由單一指標造成:低度參與群與其餘三群的差異在使用率,後三群彼此的差異則依序落在認可度與領導角色。',

    '分受眾而言,三者要處理的問題不同。人力資源部門面對的是級距與公平性:四群可直接對應四級獎金級距(圖七),辯護基礎在於級距由 KPI 實際分布劃分而非主觀判定。部門主管面對的是人力配置:專業骨幹 8 人是負責人職位最直接的候補來源,而高投入待認可群 25 人幾乎未獲指派,指派流程應於下一輪指派前逐案檢視。高階主管面對的則是結構性風險:96% 的顧問從未帶隊、領導人才集中於 4 人,此一梯隊風險的優先序高於任何個別績效問題。',

    '最後須反思結果的性質。認可度與領導角色記錄的是組織給予的機會,非顧問單方面的產出;高投入待認可群認可度偏低,可能反映指派制度而非能力。命名亦有管理後果——稱「低度參與群」或「未獲投入機會群」會導向不同行動;本報告採前者,因資料僅記錄結果。',
...
BODY['四、績效最優群組的識別'] = [
    ...
    '依據為質心表的逐欄比較(表五):三項指標同時排名第一,使用率與認可度為全體平均的2.6 與 7.4 倍。領導角色不採倍數表述——其平均僅 0.056、96% 的顧問為 0,近零分母會誇大差異;改以絕對數:樣本 107 人中僅 4 人有帶隊紀錄,恰為此群,職位合計 6 個。惟領導角色本身即為分群變數,此屬描述性特徵,不構成獨立驗證。',

    '第三節指出該二指標帶有「機會」成分,故須確認判定不倚賴它們:僅以使用率排序,群 1 仍為最高 0.810(notebook §4.2b),三種權重方案亦皆最高,成員在 20 組種子下未變動。配置建議見圖七;群 2 若政策不將帶隊列為必要條件,可與群 1 併為最高級距。',

]

BODY['五、方法限制'] = [
    '序位變數被視為等距是本方法的固有限制:以秩轉換重跑,ARI 僅 0.38,群組邊界會隨處理方式移動,惟核心發現不變。資料亦僅單一時點。',

]

BODY['結論'] = [
    '採用方案為:排除員工識別碼、施以標準化、以 k = 4 分群。標準化由三項證據支持:認可度獨佔 79.6% 距離權重、分群退化為單變數複製、10 位顧問遭錯置;k = 4 則經事前判準與敏感度檢驗。',
