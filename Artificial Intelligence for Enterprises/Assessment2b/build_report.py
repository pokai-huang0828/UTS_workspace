...
BODY['管理摘要'] = [
    ...
    '「帶隊核心群」(4 人)三項指標同時排名第一,為績效最優群,惟僅佔 3.7%。更具管理價值的是「高投入待認可群」(25 人):投入度達平均 2.2 倍但認可度低於平均,且在未標準化的分析中無法辨識。',
...
BODY['一、主要數據特徵:納入與排除的決策'] = [
    ...
    '其影響可具體量化:10 位使用率達 0.61 至 0.92 但認可度為 0 的顧問,未縮放時被併入一個 86 人的群,該群使用率跨幅 0.00 至 0.92(表二)。z 分數與 MinMax 產生相同分群,顯示結論取決於是否縮放(Milligan & Cooper, 1988)。標準化並非「不加權」,而是把權重改為可依政策調整的明確假設。',

]

BODY['二、最優聚類數的決定'] = [
    '為避免先看結果再找理由,四項判準於執行掃描前即已訂定(表三):輪廓係數(Rousseeuw, 1987)取最高、差距小於 0.02 視為平手;肘部法取降幅跌破 30% 前的 k;每群最小人數須達 4 人;群內單一 KPI 的全距須小於全體全距。後兩項為業務門檻,直接源自「依相似表現分群以分配獎金」的目的。',

    '統計面兩項互相衝突:輪廓係數最高值在 k = 5(0.6823),但 k = 3、4、5 全距僅 0.0146,依判準視為平手;肘部法則指向 k = 4,群內平方和降幅自 55.7%、44.0% 降至 29.3% 後趨平。',
    ...
    '穩定性須誠實揭露:k = 4 的種子平均調整後蘭德指數 0.961,略低於 k = 3;但僅 10 人曾變動群組,且全數落在邊界。',
...
BODY['四、績效最優群組的識別'] = [
    ...
    '依據為質心表的逐欄比較(表五):該群三項指標同時排名第一,使用率與認可度分別為全體平均的2.6 與 7.4 倍。領導角色不採倍數表述——其全體平均僅 0.056、96% 的顧問為 0,以近零數值為分母會誇大差異;改以絕對佔比陳述:這 4 人持有全公司全部 6 個帶隊職位。三項指標指向同一群,不存在 KPI 衝突,無須權重假設;另以等權、重領導、重投入三種方案檢查,群 1 皆為四群最高。',

    '此群成員在 20 組隨機種子下完全未變動,判定不受初始值影響。惟其僅佔 3.7%,建議最高級距維持 4 人以確立標竿,獎金池主體配置於前三級。群 2 僅在領導角色上落後;若政策不將帶隊視為必要條件,兩群可併為最高級距,涵蓋人數增至 12 人。',
...
BODY['結論'] = [
    '採用方案為:排除員工識別碼、施以標準化、以 k = 4 分群。標準化的必要性由認可度獨佔 79.6% 距離權重、分群退化為單變數複製、10 位高投入顧問遭錯置三項證據支持;k = 4 則由事前訂定的四項判準共同裁決。',

    '建議下一步:確認四級獎金級距與配置比例並決定是否併合群 1 與群 2、檢視指派流程是否已反映實際投入、將專業骨幹納入接班規劃。',
