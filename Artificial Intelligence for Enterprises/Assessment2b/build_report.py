...
BODY['管理摘要'] = [
    '本報告以 K-means 將 IBM 的 107 位顧問依三項績效指標分為四群,作為獎金級距設計依據。分析排除員工識別碼並施以標準化;此決策改變了 10 位顧問的歸屬,是影響最大的單一判斷。',

    '「帶隊核心群」(4 人)三項指標同時排名第一,為績效最優群,惟僅佔 3.7%。更具管理價值的是「高投入待認可群」(25 人):投入度達平均 2.2 倍但認可度低於平均,且在未標準化的分析中無法辨識,應列為指派制度檢討與留才風險的優先對象。',

]

BODY['一、主要數據特徵:納入與排除的決策'] = [
    '特徵決策分兩層:哪些變數進入距離計算,以及以何種尺度進入。',

    '排除 EmployeeID,理由有二:它是標稱型識別碼,數值不承載順序意義,納入等同主張「編號相近者表現相似」;且其標準差為其餘三欄的 36 至 103 倍,納入後獨佔距離平方 99% 以上(表一)。保留該欄實跑的反證顯示,分群會退化為按編號切段。',

    '採用標準化。常見誤解是「不標準化較中立」,事實相反:未縮放時權重由各欄離散程度決定,而非由業務判斷決定——認可度一欄即佔距離平方的 79.6%,使分群退化為單變數複製,與「直接按認可度分組」的調整後蘭德指數(Hubert & Arabie, 1985)高達 0.99。',

    '其影響可具體量化:10 位使用率達 0.61 至 0.92 但認可度為 0 的顧問,未縮放時被併入一個 86 人的群,該群使用率跨幅 0.00 至 0.92,投入 92% 與從未投入者同屬一級(表二)。z 分數與 MinMax 產生相同分群,顯示結論取決於是否縮放而非何種縮放(Milligan & Cooper, 1988)。標準化並非「不加權」,而是把權重改為可依政策調整的明確假設。',

]

BODY['二、最優聚類數的決定'] = [
    '本節檢驗四項判準:輪廓係數(Rousseeuw, 1987)、肘部法、種子穩定性與群組規模可操作性(表三、圖一)。全程固定隨機種子,以確保報告數字可重現查核。',

    '輪廓係數無法單獨定案:最高值在 k = 5(0.6823),但 k = 3、4、5 全距僅 0.0146。肘部法則指向 k = 4:群內平方和降幅自 55.7%、44.0% 降至 29.3% 後趨平。',

    '相鄰候選以業務目標排除。k = 5 產生兩個各 2 人的群,無法作為獎金級距。k = 3 的問題較隱蔽但更關鍵:其中間群 27 人,群內認可度自 0 至 4 全跨度,獲指派 4 個專案者與完全未獲指派者同屬一級,該 KPI 形同失效;k = 4 將其拆為兩群,使用率相近而認可度顯著不同,恰是獎金制度需要區分之處。',

    '穩定性須誠實揭露:k = 4 的種子平均調整後蘭德指數 0.961,略低於 k = 3;但僅 10 人曾變動群組且全數落在邊界,兩個高績效群成員完全不變。',

]

BODY['三、四個群組的語言解釋'] = [
    '分群於標準化空間進行,呈現時還原為原始單位以便解讀(表四、圖二)。聚類為描述性技術,以下呈現群組間差異,不主張因果關係。',

    '對人力資源部門:四群提供可對應的四級獎金級距,公平性的辯護基礎在於級距由三項 KPI 的實際分布劃分而非主觀判定;惟最高級距僅 4 人,建議獎金池主體配置於前三級,並為高投入待認可群設計獨立的認可機制。',

    '對部門主管:受認可的專業骨幹投入與認可俱高、尚未帶隊,是負責人職位最直接的候補來源;高投入待認可群投入度達平均 2.2 倍卻幾乎未獲指派,應於下一輪指派前逐案檢視指派流程。',

    '對高階主管:全體 96% 的顧問從未擔任負責人,領導人才集中於 4 人,人才梯隊的結構性風險高於個別績效問題,建議將專業骨幹納入接班規劃。',
...
BODY['四、績效最優群組的識別'] = [
    ...
    '依據為質心表的逐欄比較(表五):該群三項指標同時排名第一,分別為全體平均的 2.6、7.4 與 26.8 倍。三項指標指向同一群,不存在 KPI 衝突,無須動用權重假設;另以等權、重領導、重投入三種方案檢查,群 1 皆為四群最高,結論不依賴權重設定。',

    '惟此群僅佔 3.7%,獎金池若僅配置於此則涵蓋面過窄,建議最高級距維持 4 人以確立標竿,主體配置於前三級。群 2 僅在領導角色上落後;若政策不將帶隊視為必要條件,兩群可併為最高級距,涵蓋人數增至 12 人。',

]

BODY['五、方法限制'] = [
    '其一,序位變數被視為等距;以秩轉換重跑,調整後蘭德指數為 0.38,群組邊界會隨處理方式移動,惟核心發現在兩種處理下皆成立。其二,聚類為描述性技術,不能推論因果。其三,資料為單一時點,不含時間維度。',

]

BODY['結論'] = [
    '採用方案為:排除員工識別碼、施以標準化、以 k = 4 分群。標準化的必要性由認可度獨佔 79.6% 距離權重、分群退化為單變數複製、10 位高投入顧問遭錯置三項證據支持;k = 4 則由肘部法與業務可操作性共同裁決。',

    '建議下一步:確認四級獎金級距與配置比例並決定是否併合群 1 與群 2、檢視指派流程是否已反映實際投入、將專業骨幹納入接班規劃,並於補入到職年資後重跑本分析。',
...
# ═══════════════════════════ 表格資料 ═══════════════════════════
T1 = (["變數", "型別", "範圍", "標準差", "零值佔比", "佔距離權重(未縮放)", "決策"],
...
T3 = (["k", "輪廓係數", "群內平方和降幅", "最小群人數", "種子平均 ARI", "可用於獎金級距"],
      [["2", "0.6435", "—", "24", "0.801", "可,但級距過粗"],
       ["3", "0.6703", "55.7%", "4", "1.000", "否:中間群認可度 0–4 全跨度"],
       ["4", "0.6677", "44.0%", "4", "0.961", "採用"],
       ["5", "0.6823", "29.3%", "2", "0.980", "否:含兩個 2 人群"],
       ["6", "0.6708", "30.2%", "2", "0.988", "否:含兩個 2 人群"]])
...
T5 = (["群組", "使用率(排名)", "認可度(排名)", "領導角色(排名)", "等權分數", "重領導", "重投入"],
      [["帶隊核心群", "0.810(1)", "2.75(1)", "1.50(1)", "0.752", "0.745", "0.765"],
...
def build():
    ...
    table(doc, "表三　四項判準的掃描結果與裁決",
          T3[0], T3[1],
          "註:取自 notebook §2.1、§2.2、§2.4。ARI 為調整後蘭德指數,"
          "以 20 組隨機種子(100–119)與 random_state = 42 的結果比對。k = 7 至 9 同樣含 2 人群,已略。")
