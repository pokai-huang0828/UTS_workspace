...
BODY['一、主要數據特徵:納入與排除的決策'] = [
    '排除 EmployeeID:它是標稱型識別碼,數值不承載順序意義;且其標準差為其餘三欄的 36 至 103 倍,納入後獨佔距離平方 99% 以上(表一)。保留該欄實跑的反證顯示,分群會退化為按編號切段。',
    ...
    '其影響可具體量化(圖三):10 位使用率達 0.61 至 0.92 但認可度為 0 的顧問,未縮放時被併入一個 86 人的群,該群使用率跨幅 0.00 至 0.92(表二)。z 分數與 MinMax 產生相同分群(Milligan & Cooper, 1988)。標準化並非「不加權」,而是把權重改為可依政策調整的假設。',

]

BODY['二、最優聚類數的決定'] = [
    '為避免先看結果再找理由,四項判準於掃描前即已訂定,門檻與逐 k 裁決見表三:統計面為輪廓係數(Rousseeuw, 1987)與肘部法,業務面為每群最小人數與群內單一 KPI 全距。',

    '統計面兩項互相衝突:輪廓係數最高值在 k = 5(0.6823),但 k = 3、4、5 全距僅 0.0146,依判準視為平手;肘部法指向 k = 4(圖一)。',

    '業務面兩項則排除相鄰候選:k = 5 產生兩個各 2 人的群;k = 3 的中間群 27 人,群內認可度全距等於全體全距的 100%——獲指派 4 個專案者與完全未獲指派者同屬一級,該指標在此級距內無區分力,k = 4 拆開後降至 50% 以下。',

    '判準門檻本身亦須檢驗,否則仍可能是為結論量身訂做:人數門檻取 3 或 4 人結論同為 k = 4,全距判準改用對極值較不敏感的度量亦然(notebook §2.5c);惟該人數門檻無外部依據,一併揭露。',

    '穩定性亦須揭露:k = 4 的種子平均調整後蘭德指數 0.961 略低於 k = 3,但僅 10 人曾變動且全在邊界。',
...
BODY['三、四個群組的語言解釋'] = [
    ...
    '對部門主管:受認可的專業骨幹尚未帶隊,是負責人職位最直接的候補來源;高投入待認可群投入度達平均 2.2 倍卻幾乎未獲指派,應檢視指派流程。',

    '對高階主管:樣本中 96% 的顧問從未擔任負責人,人才梯隊風險高於個別績效問題。',

    '最後須反思結果的性質。認可度與領導角色記錄的是組織給予的機會,非顧問單方面的產出;「高投入待認可群」認可度偏低,可能反映指派制度而非能力。命名亦有管理後果——稱「低度參與群」或「未獲投入機會群」會導向不同行動;本報告採前者,因資料僅記錄結果。',
...
BODY['四、績效最優群組的識別'] = [
    ...
    '依據為質心表的逐欄比較(表五):三項指標同時排名第一,使用率與認可度為全體平均的2.6 與 7.4 倍。領導角色不採倍數表述——其平均僅 0.056、96% 的顧問為 0,近零分母會誇大差異;改以絕對數:樣本 107 人中僅 4 人有帶隊紀錄,恰為此群,其職位合計 6 個。惟領導角色本身即為分群變數,此屬該群的描述性特徵,不構成獨立驗證。',

    '第三節指出認可度與領導角色帶有「機會」成分,故須確認判定不倚賴此二者:僅以使用率排序,群 1 仍為最高 0.810,結論不變(notebook §4.2b);另以三種權重方案檢查,群 1 亦皆為最高。',

    '此群成員在 20 組隨機種子下完全未變動。惟其僅佔 3.7%,配置建議見圖七;群 2 僅在領導角色上落後,若政策不將帶隊列為必要條件,兩群可併為最高級距(12 人)。',
...
BODY['結論'] = [
    '採用方案為:排除員工識別碼、施以標準化、以 k = 4 分群。標準化的必要性由認可度獨佔 79.6% 距離權重、分群退化為單變數複製、10 位高投入顧問遭錯置三項證據支持;k = 4 由事前判準裁決並經門檻敏感度檢驗。',

    '建議下一步:確認獎金級距與配置比例(圖七)、檢視指派流程、將專業骨幹納入接班規劃。',
