...
2. 🔑 **成績不能只看絕對分數**。88.75 對 81.5 看似大進步,但老師課堂說「**平均分兒都已經接近 90**」
   —— Kenny 兩次都在班平均下方一點,相對位置沒改善。**拿到班級分佈才能解讀分數。**
3. ✅ **對抗驗證確實抓得到實質錯誤**,不只是修辭。三個 verifier 全 FAIL,
   揪出的是邏輯矛盾與虛構細節(發酵溫度曲線、糖度、pH、2010 年紀錄等題目沒有的東西),不是句式問題。
4. ✅ **A1 復盤機制被證明有效**:復盤第 4 條行動清單照做 → 策略制定 25/25,是全份唯一破格項。
   同時驗證了 A1 復盤標為「信心中等」的推論「老師偏好落地型」——
   唯一落地型評分項滿分,四個分析型全停在 85% 門檻。
5. ✅ **老師點名的兩個 A2a 共通問題,我方都避開了**:建模流程前後不一致、FN 成本用全體平均月費。
   我方用的是「依 FN 個體實際月費 A$2,993.8」,正是老師要的做法。
   → 17/20 不是算錯,是檔次邊界值現象。
6. ℹ️ **Canvas 長內容抓法**(SKILL §0 已記載,本次實用有效):`javascript_tool` 單次回傳約 1000 字元上限,
