---
name: uts-dispatch
description: 指揮官派工守則(UTS_workspace)— 主模型當指揮官,派工給 Codex subagents 生產、Codex(gpt-5.6 異家族)對抗驗證。多步驟任務、批次讀檔、事實類交付、需要第二意見時使用。
---
...
| 通道 | 可用值 | 備註 |
|------|--------|------|
| Agent tool `model` | `haiku` / `sonnet` / `opus` / `fable` | 無 effort 參數。**2026-07-25 實查:`opus` = `Codex-opus-5`,與主迴圈同型號**,派它不是「比較強」,是 context 乾淨 / 可平行 / 可 worktree 隔離 |
...
核心:**Codex 生產、Codex(異家族)驗證** —— Codex 抓到的錯多是「Codex 沿用自己舊筆記」型,自驗有共同盲點。完整配方見 [references/codex-collab.md](references/codex-collab.md)。

```text
指揮官(主迴圈 = Opus 5)
 ├─ 生產線:Explore(haiku)搜 → general-purpose(sonnet)寫 → opus(乾淨 context)判斷題
 ├─ 驗證線【並行,同一則訊息發出,不排隊】
 │   ├─ 軌①語意面:fresh Codex(sonnet)— 有沒有漏講、rubric 有沒有蓋到、論證缺口
