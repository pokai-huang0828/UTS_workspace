...
> 最後更新:**2026-09-10**(9/9 第 2 週 Zoom:單元二測驗規格 + 夏令時陷阱 + 單元一答案全公布);
> 前次 **2026-08-25**(新增 321513 Machine Learning 區塊);再前次 **2026-08-13 23:52**(A3 對外簡報版完成：全份改為外部主管可理解的提案語氣，表格／路線圖／文獻頁重排，並保留微軟正黑體可編輯性；待辦見下方 A3 區塊)
...
| **2026-09-20 週日 23:59** | 🔴 單元二測驗(**9/16 週三開放**;10 題 / **60 分鐘** / 只有一次機會)<br>⚠️ **第 1 題不是選擇題** —— 要下載 `.ipynb` 補全程式碼並執行,跑出決策樹 vs 隨機森林準確率<br>⚠️ 範圍涵蓋**第 2 週 + 第 3 週**(老師 9/9 口述) |
...
- [ ] 🔴 **每場 Zoom 都要記老師拋出的討論情境** —— 9/3 那場證明老師舉的兩個例子
      ...
      - [x] ~~9/3 第 1 週~~ ✅ 已落檔 ｜ [x] ~~9/9 第 2 週~~ ✅ **2026-09-09 已落檔**
      - 🎁 **9/9 那場老師逐題公布了單元一測驗全部 10 題的答案**,已完整還原進 `91_上課紀錄.md`
        (含全班正確率只有 39% 的「訂閱服務場景」題)
      - 🔑 9/9 的課堂討論題:**監督 vs 非監督的判準只有一條 —— 訓練資料有沒有標註**;
        陷阱是「**分類=監督、分群=非監督**」
- [ ] 🔴 **9/16 前**:確認單元二測驗第 1 題的 Notebook 跑得動(開 VS Code 選 kernel 跑一次
      決策樹+隨機森林即可,sklearn 內建、不需裝任何東西)
      - ⏱️ 測驗規格:**10 題 / 60 分鐘 / 只有一次機會 / 可跳題**(頁面上方有題號列表)
      - 📌 應考策略(課堂上其他學生的建議,老師採納):**第 1 題先跳過,把後面選擇題做完再回頭**
- [ ] 🟡 **盯 Canvas 公告**:老師答應會公布單元二測驗第 1 題 Notebook 需要的套件清單
