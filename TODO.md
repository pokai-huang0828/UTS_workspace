...
> 最後更新:**2026-09-15**(9/10 第 2 週第二場 Zoom:單元二偏技術觀念 + 多選辨識 + A3 規定可能改 + Notebook 洩漏兩處);
...
| ~~2026-09-20 週日 23:59~~ ✅ **已於 9/16 作答** | 單元二測驗(**9/16 週三開放**;10 題 / **60 分鐘** / 只有一次機會)<br>⚠️ **有題目要下載 `.ipynb` 補程式碼並執行**(9/9 說第 1 題、9/10 說「有的題」→ 可能不只一題)<br>⚠️ 範圍涵蓋**第 2 週 + 第 3 週**;老師建議**讀完第 3 週再做**<br>⭐ **偏重技術觀念、不是商業案例**(和單元一不同,9/10 口述)<br>⭐ **有多選題:方框 ☐ = 多選、圓圈 ○ = 單選**<br>⭐ **用筆電做,不要用平板/手機** |
...
- [ ] 🔴 **每場 Zoom 都要記老師拋出的討論情境** —— 9/3 那場證明老師舉的兩個例子
      ...
      - [x] ~~9/3 第 1 週~~ ✅ 已落檔 ｜ [x] ~~9/9 第 2 週~~ ✅ **2026-09-09 已落檔** ｜ [x] ~~9/10 第 2 週第二場~~ ✅ **2026-09-15 已落檔**
      ...
      - 🔑 9/9 的課堂討論題:**監督 vs 非監督的判準只有一條 —— 訓練資料有沒有標註**;
        陷阱是「**分類=監督、分群=非監督**」
- [x] ~~單元二測驗~~ ✅ **2026-09-16 已作答**(開放當天晚上)。實測心得已寫進 `README.md` §3.1–3.3 與 91 的 9/10 entry:
      - ⚠️ **題幹的「單選/可多選/哪些」不可信,一律看選項框**:Q3 寫單選卻是方框(可多選)、Q10 寫「哪些」卻是圓圈(單選)
...
- [ ] 🟡 **盯 Canvas 公告**:
      - 單元二測驗 Notebook 需要的套件清單(9/9 老師答應公布)
...
- [ ] 🟢 **README 課程總整理:最後一道 Codex 全件終審(可選)** —— 2026-09-16 已由**獨立 Claude agent** 做完缺陷關閉 + 全件事實掃描,只剩 2 處出處標註問題且已修。
      仍缺的只有:**§8 考點地圖 19 列**僅由 Codex 第 1 輪核對過(Claude agent 因讀檔上限沒重驗)。
      Codex 全件終審兩度撞用量上限(9/16 兩趟共 16.3 萬 token,重置 9/17 00:40);跑完拿到 READY 就可以拿掉 README 頂端的查證狀態提示
