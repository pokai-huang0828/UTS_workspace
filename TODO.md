...
> **10/04 之前**雪梨 +2 小時(23:59 = 台灣 21:59);**10/04 之後** +3 小時(23:59 = 台灣 **20:59**)。
> A2(10/04)與 A3(10/12)剛好卡在切換前後,別算錯。
...
- [ ] 🟡 **A3 照官方骨架補完,不換模型、不重寫 Notebook** —— 9/10 老師判斷「**大概率不太行**」(開會確認中);
      想法和改進寫在報告裡。評分看「**建構過程嚴不嚴謹 + 報告好不好**」,不是分數多高
- [ ] 🟢 **A3 報告素材(待獨立驗證)**:Bank X 官方 Notebook 兩處違反 9/10 老師講的洩漏原則 →
      **cell 26** 測試集重新擬合 power_transform、**cell 58** 交叉驗證包在 Pipeline 最後一步(RFE 先看過驗證折)
      → 詳見 `Machine Learning/data/bank_x/_Notebook拆解.md` 末段。**寫進報告前先過 Codex 核對**(目前只有自查)
