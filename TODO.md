...
| **2026-09-20 週日 23:59** | 🔴 單元二測驗(**9/16 週三開放**;10 題 / **60 分鐘** / 只有一次機會)<br>⚠️ **有題目要下載 `.ipynb` 補程式碼並執行**(9/9 說第 1 題、9/10 說「有的題」→ 可能不只一題)<br>⚠️ 範圍涵蓋**第 2 週 + 第 3 週**;老師建議**讀完第 3 週再做**<br>⭐ **偏重技術觀念、不是商業案例**(和單元一不同,9/10 口述)<br>⭐ **有多選題:方框 ☐ = 多選、圓圈 ○ = 單選**<br>⭐ **用筆電做,不要用平板/手機** |
...
- [ ] 🔴 **每場 Zoom 都要記老師拋出的討論情境** —— 9/3 那場證明老師舉的兩個例子
      ...
      - 🔑 9/9 的課堂討論題:**監督 vs 非監督的判準只有一條 —— 訓練資料有沒有標註**;
        陷阱是「**分類=監督、分群=非監督**」
- [ ] 🔴 **9/16 前**:確認單元二測驗要用的 Notebook 跑得動(開 VS Code 選 kernel 跑一次
      決策樹+隨機森林即可,sklearn 內建、不需裝任何東西)
      - ⏱️ 測驗規格:**10 題 / 60 分鐘 / 只有一次機會 / 可跳題**(頁面上方有題號列表)
      - 📌 應考策略(課堂上其他學生的建議,老師採納):**要跑 Notebook 的題先跳過,把選擇題做完再回頭**
      - 📌 9/10 補充:**用筆電**、**看到方框 ☐ 就是多選**(同學單元一漏看多選被扣 0.5 分)
- [ ] 🔴 **9/20 前讀完第 3 週**,讀書重點放**技術觀念**(樹模型、邏輯迴歸、損失函數、學習率、過擬合、k 折交叉驗證)
