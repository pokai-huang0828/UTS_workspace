...
> 最後更新:**2026-08-08 晚**(A3 全案換題為公司車隊 AI 案,v4 地基完成,內容產出中)
...
>
> ### 🔴 待辦(依優先序)
> - [ ] **① 補跑第二路對抗**(判準覆蓋與說服力,因 API 連線中斷失敗)
>       `Workflow({scriptPath:'…/workflows/scripts/a3-v4-content-draft-wf_cd7cebca-d44.js', resumeFromRunId:'wf_92328b60-bee'})`
>       其餘 8 支走快取,只會重跑失敗那一支
> - [ ] **② 一次改完十頁** —— 帶著兩個鏡頭的意見 + 上面三個新事實。
>       五個 fatal 中 F1/F2/F3/F5 已由新事實解掉,**F4(四個工作包沒有 AI)仍需改寫**:
>       P1 明講 AI 能力、規則式那包改寫成「WP-E 的對照基線」、請求改成六個月整體額度分段釋出
> - [ ] ③ 出圖(★ 兩個 30 分項各至少一個專業視覺化)→ 逐頁渲染稽核
> - [ ] ④ 組版(機具可沿用,見 `notes/03_出圖與組版機具.md`)
> - [ ] ⑤ 講稿定稿 + 配時程式驗算
