...
> 最後更新:**2026-08-12 22:30**(A3 內容 v2.5 定稿、十張圖與 pptx 已重建;待辦見下方 A3 區塊)
...
>
> ### 🔴 待辦(依優先序 · **狀態更新至 2026-08-12 22:30**)
>
> ✅ 已完成:內容十頁 v2.5 定稿(notes)· 十張圖 · 組版機具 · 配時驗算(2,464 字 = 560.0 秒 = 9:20,餘裕 40 秒)·
> fig 02/05/09/10 依 v2.5 重出 · **pptx 重建**(`build_deck_v4.py` 自 `7cbd049` 攤平目錄後一直是壞的,已修)
>
> - [ ] 🔴 **① v2.5 落地到出圖腳本與 deck** —— 第二/三/四批**只改了 notes,一行 deck 都沒改**。
>       實掃重建後的 pptx,下列七項**完全不在交件物裡**:
>       ⑬ 漏濾率四張卡 · R9/R10 兩條自動結案風險 · 八原則雙欄評等 ·
>       可避免 NT$520-1,521 萬(被減數 11-12 人)· 稽核稅 0.39-3.14 FTE ·
>       三年 TCO 雙軌(50-118 / 173-1,219 萬)· G4 移交後獨立門。
>       落點:`fig_v4_06/07/08/09/10.py` + `scripts/deck/slides_05_07.py` `slides_08_12.py`
> - [ ] 🔴 **② 兩處母版打架,要 Kenny 裁**:
>       (a) P9 圖上仍印 `Qwen3.5 122B-A10B(4-bit)`(P9 視覺規格明文要求),但 P5 已依裁決 AA 拿掉 →
>           **同一份簡報兩個層次**,選一個;
>       (b) P8 卡 4:`P08-11.md` 第 23 行寫可避免 = NT$520-1,521 萬,第 90-93 行仍寫 1,248-2,223 萬
> - [ ] ③ **PowerPoint COM 匯出 PNG 逐頁目視**(會啟動 PowerPoint、搶焦點,Kenny 在場時才跑)
> - [ ] ④ 組版 5 項修版:P10 橫幅出血 · P5 三元素貼疊 · P6 灰字牆貼線 · P7 右上註記溢出 · P12 header 重疊
> - [ ] ⑤ **獨立複驗**(主迴圈是產出者,不能自己驗自己)+ Codex 異家族審 v2.5
