...
> - [ ] **④ 🔴 修三張圖的文字重疊/裁切** —— 講稿定稿時實際開檔核圖發現:
>       `fig_v3_02_gap`、`fig_v3_08_ethics`、`fig_v3_10_resource` 有文字重疊或被裁切,
>       **需修版後才能進投影片**。修完重跑 `make_figs_v3.py` + `build_deck_v3.py`,
>       再用 PowerPoint COM 導出逐頁複核
> - [ ] **⑤ 🔴 主檔 P6/P7 時序對不上** —— P6 的 WP0-D(W8 出實驗結果報告)與
>       P7 的「到期後 90 日」量測窗不相容。**講稿已改口徑為「分段結果報告」並在答辯 Q12 講清楚,
>       但 v3 主檔尚未同步回填** —— 不改,講稿與主檔就不是同一事實來源
> - [ ] **⑥ ⚠️ 紅線檢查只跑了 2/3** —— 講稿 workflow 的 `verify:紅線與痕跡` 因 API Error 失敗。
>       finalize 自己掃過(宣稱 0 命中)**但那是產出者自檢**,需獨立補一次
> - [ ] ⑦ 參考文獻頁填入**實際 APA 條目**(現在是說明文字)
> - [ ] ⑧ 彩排(兩次時長差 ≤15 秒、都 ≤9:40、無需停下重來)
> - [ ] ⑨ **逐頁錄製**(非一鏡到底)→ MP4 → **8/17 白天交件,不要排在當天**
>
> ### ✅ 已修正(2026-08-08 09:55)
> - **配時**:v3 §〇 原表逐列相加 **580** 卻標「~560」,且頁標題用了第三組數字。
>   已用講稿實測值(2,434 等效字 ÷ 4.4 字/秒)整表覆蓋,程式驗算 = **560 秒 = 9:20**,
>   表格與 12 個頁標題完全一致。⚠️ **這是同類錯誤第二次**(上一輪是加總錯 100 秒)
> - **圖檔名對照**(先前給錯):甘特圖在 **`fig_v3_05_plan`(P6)**;
>   `fig_v3_10_resource` 是人月長條圖,**沒有 `fig_v3_10_gantt` 這個檔**
> - **fig_v3_01 沒有 Evident 排名那一列** —— 該項改為口述,不可依賴圖
>
> ### 📌 講稿定稿的關鍵數字(供覆核)
> 560 秒 = 9:20(實需 557.6 秒);逐頁負荷率全部 ≤100.1%;語速 **4.4 字/秒**
> (下限 4.1,再慢破 10:00);距 600 秒硬上限餘裕 42.4 秒。
> **最弱頁 = P7 實驗設計**(55 秒、負荷 100.0%、統計術語連發、零緩衝,彩排要優先卡這頁)。
>
> ### ✅ 已驗證的關鍵事實(可直接引用)
